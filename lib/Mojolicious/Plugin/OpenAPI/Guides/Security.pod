...
"security" under the given HTTP method. Call C<$cb> with C<undef> or no
argument at all to indicate pass. Call C<$cb> with a defined value to indicate
that the check has failed. Note that the callback must be called or the
dispatch will hang.
